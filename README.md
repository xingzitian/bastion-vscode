...
## 🔌 让外部 AI 直接用你的堡垒机会话（MCP）

**这是本扩展最被低估的功能**：它同时是一个 **MCP 服务端** —— 于是 Claude Desktop、Trae、CodeBuddy、
Cursor 或者任何支持 HTTP MCP 的客户端，都能直接操作你**已经登录好的**堡垒机会话：跑命令、读屏幕、传文件。

**不用装 node、不用 npm、不用再跑转发器 —— 有 VS Code 就够了。** VS Code 开着，端点就在跑。

一键步骤：

1. 命令面板 → **`BastionShell: 让外部 AI 连上（MCP 端点 / 一键复制配置）`**
2. 挑一项复制（端点+token / VS Code 手配片段 / 其它客户端片段）→ 粘进那个 AI 客户端的配置里
3. 回到 AI 里让它「先调 `bastion_listSessions` 看看有哪些会话」

它比"给 AI 一个终端"强的地方在于：**会话是人和 AI 共用同一条**。MFA、选资产、输密码这些必须
由人做的步骤你在终端里做完，AI 直接在**同一条会话**上接着干 —— 既不用把密码/动态码交给 AI，
也不用重新认证。另外它自带几道闸：高危命令（删根、格式化、写块设备）**直接不发出去**、
会话停在堡垒机菜单上时**拒绝发命令**（发出去会被菜单吃掉）、只读工具标了 `readOnlyHint`
（客户端不会为它弹确认框）。

> 端点信息（URL / token）在 `~/.bastionshell/mcp.json`；token 是长期有效的，重载窗口不会变。
> 只有**本机**能连（只监听 127.0.0.1）。关掉：设置 `bastion.mcpEnabled`。
> 详细说明与排查见下文「MCP」一节。
